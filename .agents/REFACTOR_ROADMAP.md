...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5-sprint-20260506` — **Plan gate:** recon → planner → plan-auditor (**NEEDS_WORK**, amended): **5e** reframed — `model/ligand_mpnn.py` **already hosts** ligand MPNN (~§5e extraction done); remaining **5e-cont** is **`mpnn.py` shrink / façade** toward §11 LoC target. **Sequence:** **PR3a** sampling `io_callback`/`effects_barrier` consistency (same risk class as PR1; preserve post-concat perplexity contract) **before** **PR3b** scoring tensor D2H / sink unify. **5f** (`jacobian` / `conformational_inference` driver breadth) **after** PR3a semantics stable. **jaxbeans DEPEND:** blocked until packaging policy — `tev_design` **workspace members** are **`prxteinmpnn` + `prolix` only** (see `pyproject.toml`); until jaxbeans is added as member or path/git dep, keep **`utils/safe_map.py`** / **`utils/atomic_write.py`** in-repo mirrors (documented). **Prior landed:** `refactor-phase5-sprint-20260509` (**PR2c** averaged scoring markers + concat-anchor loop reconciliation). |
| **Last update** | **2026-05-06** — Sprint **plan** recorded; **no code slice merged in this planning commit** beyond docs/`pyproject` note. **Still defer:** chunk/tensor streaming; HDF5 vs ArrayRecord adapter; jaxbeans package wiring; ``OUTPUT_SINKS``; **5h**. |
| **Current phase** | Phase 5 **5g** follow-through (PR3a → PR3b) + **5f** breadth + **5e-cont** `mpnn.py` decomposition. |
| **Still open** | Stream **chunk tensors** + unify HDF5/ArrayRecord sinks; jaxbeans **DEPEND** swap when workspace/policy allows; optional ``OUTPUT_SINKS`` + driver breadth on ``run/jacobian.py`` / ``run/conformational_inference.py``; scoring tensor streaming; **`mpnn.py` §11 LoC** target; **5h** ensemble relocation (Q3). |
| **Plan** | **Verification cwd:** package root `prxteinmpnn/` (`tests/...` paths; `pythonpath` from `[tool.pytest]`). **Merge order:** (0) packaging note / DEPEND criteria; (1) **5e-cont**; (2) **PR3a** sampling barriers; (3) **PR3b** scoring tensors; (4) **5f**. **Reference sprint:** `.agents/SPRINT_refactor-phase0a-go-pr2-sample-20260507.md`. |
