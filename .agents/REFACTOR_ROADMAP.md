...
## 14. Sprint status (Phase 0a GO; Phase 5g **PR4 sampling tensor hook landed** — **next: sink unify + 5e-cont**)

| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5-sampling-stream-20260507` — **landed PR4 (2026-05-07):** ``run/sampling.py`` — ``_noop_sampling_tensor_batch_io`` + tensor ``io_callback`` after intra-batch ``jnp.concatenate`` (with optional structure-batch scalar first when ``emit_structure_batch_io``); both ``ordered=False``, ``stop_gradient`` on operands. Tensor hook runs every ``_sample_batch`` exit (campaign intermediate chunks included); structure scalar remains last-chunk-only (PR3a). Streaming HDF5 / ArrayRecord still ``np.asarray`` (**second D2H** vs hook — documented). Tests: ``tests/streaming/test_sampling_tensor_batch_io_callback.py``. Sprint plan: ``.agents/SPRINT_refactor-phase5g-pr4-20260507.md``. Verification: ``.agents/verification_logs/sprint_phase5g_pr4_sampling_tensor_20260507.filtered.txt``. **Prior:** PR3b scoring tensors (``refactor-phase5-pr3b-sprint-20260507``), PR3a sampling markers. **Next:** HDF5 vs ArrayRecord sink unify / optional per-chunk tensor streaming (narrow PRs); **5e-cont**; **5f**; jaxbeans DEPEND. |
| **Last update** | **2026-05-07** — Phase **5g PR4** sampling per-batch tensor ``io_callback``. **Still defer:** per-chunk tensor streaming inside ``_sample_batch`` loop (vs post-concat hook); HDF5/ArrayRecord adapter + ``OUTPUT_SINKS``; jaxbeans package wiring; **5h**. |
| **Current phase** | Phase 5 **5g**: PR4 closed → **sink follow-ups** + **5f** breadth + **5e-cont**. |
| **Still open** | Unify HDF5/ArrayRecord host sinks with tensor hooks (remove double D2H); optional **chunk-level** tensor ``io_callback`` (contract vs perplexity — see ``TODO_io_callback.txt``); jaxbeans **DEPEND** swap when workspace/policy allows; ``OUTPUT_SINKS`` + driver breadth on ``run/jacobian.py`` / ``run/conformational_inference.py``; **`mpnn.py` §11 LoC** target; **5h** ensemble relocation (Q3). |
| **Plan** | **Verification cwd:** package root `prxteinmpnn/`. **Merge order:** sink / adapter small PRs; **5e-cont**; **5f**. Logs: PR3a ``.agents/verification_logs/sprint_phase5g_pr3a_sampling_io_20260506.filtered.txt``; PR3b ``.agents/verification_logs/sprint_phase5g_pr3b_20260507.filtered.txt``; PR4 ``.agents/verification_logs/sprint_phase5g_pr4_sampling_tensor_20260507.filtered.txt``. |
| **Next action (no prior context)** | Read §14 **Still open** + `TODO_io_callback.txt`. **Do next:** host sink unification or **5e-cont** / **5f** (parallel); jaxbeans DEPEND after packaging decision. **Deferred:** **5h** (Q3). Phase **6** waits on Phase 5 host boundaries — §390–416. |
