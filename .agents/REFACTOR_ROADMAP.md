...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5e-ligand-encoder-20260506` — LigandMPNN AR scan + ligand ``state_vmap_exact`` encoder context folded through ``pack_encoder_context`` (mirrors protein path). Prior: `refactor-phase5a-encoder-20260506` (``pack_encoder_context`` introduction + protein wiring). |
| **Last update** | **2026-05-06** — **Phase 5e:** ``PrxteinLigandMPNN._run_autoregressive_scan`` and ``sample_autoregressive_state_vmap_exact`` call ``pack_encoder_context`` / ``jax.vmap(pack_encoder_context)`` instead of duplicated ``concatenate_neighbor_nodes`` + ``mask_fw`` scaling; docstring on ``pack_encoder_context`` updated. Verification: ``.agents/verification_logs/phase5e_ligand_encoder_20260506.filtered.txt``. **Earlier:** Phase 5a helpers + protein mirrors; Phase 5f ``SamplingDriver.build_sampler_fn``; Phase 4 routing tests. |
| **Current phase** | Phase 4: routing regression tests landed; **remaining Phase 4 chunk** = optional ``state_vmap_exact`` **body** dedup / vmap reference extension for AR wave and deeper ligand paths (beyond routing parity). Phase 5: **5a + 5e (ligand packing mirrors) landed**; **still open:** decoder/core/shared splits (5b–5d), deeper ligand orchestration beyond these two mirrors, io_callback streaming (5g), jaxbeans ``safe_map`` DEPEND swap. |
| **Still open** | ``state_vmap_exact`` implementation unification where desired; **remaining** ``run/sampling.py`` internals (_sample_streaming variants, averaged path wiring) vs full host-driver consolidation; ``utils/_vendored_callbacks.py`` + barriers. **Deferred to Phase 6 §15:** batch layout policy. |
