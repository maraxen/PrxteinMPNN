...
## 14. Sprint status (Phase 0a GO; Phase 5g **PR3b scoring tensor hook landed** — **next: sampling tensor / sink unify + 5e-cont**)

| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5-pr3b-sprint-20260507` — **landed PR3b (2026-05-07):** ``run/scoring.py`` — ``_noop_scoring_tensor_batch_io`` + ``_scoring_structure_and_tensor_batch_io`` (scalar marker then tensor ``io_callback``, both ``ordered=False``, ``stop_gradient`` on operands) in ``_score_standard_mode`` / ``_score_batch_averaged`` / ``_score_streaming`` traced forwards; non-streaming ``score()`` concat contract unchanged; streaming HDF5 keeps ``np.asarray`` (**documented double D2H** until sink unification). Tests: ``tests/streaming/test_scoring_tensor_batch_io_callback.py``. Sprint plan: ``.agents/SPRINT_refactor-phase5g-pr3b-20260507.md``. Verification: ``.agents/verification_logs/sprint_phase5g_pr3b_20260507.filtered.txt``. **Next:** sampling-side tensor streaming / HDF5 vs ArrayRecord adapter (when scope stays small); **5e-cont**; **5f**; jaxbeans DEPEND. **Prior:** PR3a sampling (`refactor-phase5-sprint-20260506`), PR2c scoring averaged markers. |
| **Last update** | **2026-05-07** — Phase **5g PR3b** scoring per-batch tensor ``io_callback``. **Still defer:** chunk tensor streaming on ``run/sampling.py`` (`_sample_streaming*` paths); HDF5 vs ArrayRecord adapter; jaxbeans package wiring; ``OUTPUT_SINKS``; **5h**. |
| **Current phase** | Phase 5 **5g**: PR3b closed → **sampling tensor / sink follow-ups** + **5f** breadth + **5e-cont**. |
| **Still open** | Stream **chunk tensors** on sampling + unify HDF5/ArrayRecord sinks; jaxbeans **DEPEND** swap when workspace/policy allows; optional ``OUTPUT_SINKS`` + driver breadth on ``run/jacobian.py`` / ``run/conformational_inference.py``; **`mpnn.py` §11 LoC** target; **5h** ensemble relocation (Q3). |
| **Plan** | **Verification cwd:** package root `prxteinmpnn/`. **Merge order:** sampling streaming tensors / adapter (small PRs); **5e-cont**; **5f**. Logs: PR3a ``.agents/verification_logs/sprint_phase5g_pr3a_sampling_io_20260506.filtered.txt``; PR3b ``.agents/verification_logs/sprint_phase5g_pr3b_20260507.filtered.txt``. |
| **Next action (no prior context)** | Read §14 **Still open** + `TODO_io_callback.txt`. **Do next:** sampling ``_sample_streaming`` / ``_sample_streaming_arrayrecord`` tensor materialization + optional adapter unify (**keep PR theme narrow**); parallel plannable **5e-cont**, **5f**, jaxbeans DEPEND after packaging decision. **Deferred:** **5h** (Q3). Phase **6** waits on Phase 5 host boundaries — §390–416. |
...
*End of roadmap. Phase 2 typed boundaries are landed; Phase 3b portable RunSpec JSON v2 + hygiene signed off; **2026-05-07** landed Phase **5g PR3b** scoring tensor ``io_callback`` + filtered verification log; prior **2026-05-07** sprint closed ``SAMPLERS`` / ``SamplingDriver`` prep + Jacobian/CIF/Inspection JSON round-trips + Phase 4 unconditional-path documentation (§13.2 GO). **2026-05-06** closed PR2b host coercion for ``make_sample_sequences`` temperature path (loose stack kwargs). **Phase 6** now includes batch-layout / memory policy (**§15**) after Phase 5. Long-form sampling notes remain under `.agents/SPRINT_refactor-phase0a-go-pr2-sample-20260507.md`.*
