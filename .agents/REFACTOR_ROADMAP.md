...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5-sprint-20260507` — **Plan gate:** recon → planner → plan-auditor (**NEEDS_WORK** → PR1-only merge); **landed:** ``_sample_batch`` per-chunk scalar marker ``jax.experimental.io_callback(..., ordered=False)`` + ``jax.effects_barrier()`` immediately before ``jnp.concatenate`` (host hook ``_noop_sampling_chunk_io``, test-monkeypatchable); ``tests/streaming/test_sampling_chunk_io_callback.py``; ``TODO_io_callback.txt`` PR1 scope note (scoring / jacobian / conformational + HDF5/ArrayRecord adapter **deferred**). Prior: ``refactor-phase5-streaming-contract-20260506``. |
| **Last update** | **2026-05-07** — **5g PR1:** chunk-marker ``io_callback`` on-device path **without** tensor D2H or concat semantics change (ppx / ``return_logits`` contract unchanged). **Still deferred:** chunk tensor streaming; HDF5 vs ArrayRecord adapter PR; jaxbeans **DEPEND** swap beyond in-repo ``safe_map`` shim; ``OUTPUT_SINKS`` registry; **5h**. |
| **Current phase** | Phase 5 **5g**: sampling chunk markers + barrier pre-concat; follow-ups per ``TODO_io_callback.txt``. |
| **Still open** | Stream **chunk tensors** + unify HDF5/ArrayRecord sinks (post-concat host payloads); jaxbeans **DEPEND** packages (multipart writers, profiling/io where adopted); optional ``OUTPUT_SINKS`` registry + driver breadth on ``run/jacobian.py`` / ``run/conformational_inference.py``; **5h** ensemble relocation (Q3). |
