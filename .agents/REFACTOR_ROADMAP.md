...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5bcdg-20260506` — **5b–5d:** decoder packing helpers, ``mpnn_core`` AR gathers, ``_shared`` multistate logits + group tables; **5g slice:** ``utils/_vendored_callbacks.py``, ``multistate_pools`` ``io_callback(..., ordered=False)`` + ``effects_barrier``, ``tests/streaming/test_io_callback_ordering.py``. Prior: `refactor-phase5e-ligand-encoder-20260506` (ligand ``pack_encoder_context`` mirrors). |
| **Last update** | **2026-05-06** — Landed mechanical extractions for roadmap **5b / 5c / 5d** (decoder / mpnn_core / _shared) plus **5g** scaffolding (vendored callbacks module, pool barrier pattern, CI smoke tests). Verification: ``.agents/verification_logs/phase5bcdg_mpnn_split_streaming_20260506.filtered.txt``. **Not in this slice:** Phase **5e** file split (``ligand_mpnn.py``), full ``run/sampling.py`` io_callback plumbing (atomic_write / MultiPartWriter / ``safe_map`` DEPEND), jaxbeans-heavy 5g remainder. |
| **Current phase** | Phase 4: routing regression tests landed; optional ``state_vmap_exact`` body dedup remains. Phase 5: **5b–5d** landed (decoder / ``mpnn_core`` / ``_shared``); **5g** partially landed (vendored callbacks + pool barrier + streaming tests; not full ``run/sampling.py`` / jaxbeans writer stack). **Earlier milestones still relevant:** 5a encoder helpers, ligand ``pack_encoder_context`` mirrors, 5f ``SamplingDriver`` wiring. **Still open:** roadmap **5e** *file split* (``ligand_mpnn.py`` + shrunk ``mpnn.py``), remaining **5g** host I/O (``safe_map``, atomic writers per §3.6), full streaming dedup in ``run/sampling.py``. |
| **Still open** | Phase **5e** mechanical split of ``PrxteinMPNN`` vs ``PrxteinLigandMPNN`` into separate modules; ``run/sampling.py`` / ``run/scoring.py`` io_callback paths beyond ``multistate_pools``; ``safe_map`` DEPEND swap; optional cold-start JIT regression note (roadmap critic #6). **Deferred:** Phase 6 §15 batch layout policy. |
