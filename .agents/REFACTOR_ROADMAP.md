...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5-closure-20260506` — **5e:** ``model/ligand_mpnn.py`` extraction + ``mpnn.py`` re-exports; multistate helpers call ``_shared`` free functions (no cross-class static hops). **5g slice:** ``jax.effects_barrier()`` after ``_sample_batch`` / averaged paths in ``run/sampling.py`` (pairs with ``ordered=False`` ``io_callback`` policy elsewhere). **Writers:** ``utils/atomic_write.py`` + ``campaign_manifest.write_manifest``. **5f:** ``run/scoring_driver.py`` + scoring batch barriers; Jacobian / conformational batch barriers. **Docs:** ``safe_map`` module header notes jaxbeans DEPEND deferral. **Smoke:** ``tests/profiling/test_cold_start.py``. Prior: ``refactor-phase5bcdg-20260506``. |
| **Last update** | **2026-05-06** — Verification: ``.agents/verification_logs/phase5_closure_20260506.filtered.txt`` (40 passed, 3 skipped). **Not landed here:** full ``io_callback`` D2H streaming inside ``_sample_batch`` (still ``np.asarray`` / TODO paths); jaxbeans **DEPEND** swap for ``safe_map`` / ``PreemptionHandler`` / ``BinaryDatasetWriter``; ``SamplingDriver`` dedup of **all** ``run/sampling.py`` variants into sink registry; **5h** ensemble→jaxbeans. |
| **Current phase** | Phase 5: **5e file split landed**; **5g** advanced on **host staging** (barriers on sampling/scoring/Jacobian/conformational batch boundaries + manifest atomic write). **5f** expanded with **``ScoringDriver``** (parallel to ``SamplingDriver``). **Still open:** JIT-tail ``io_callback`` design-writer integration per ``TODO_io_callback.txt``; registry-style score factories (optional); Phase 6 §15 batch layout. |
| **Still open** | Deep ``run/sampling.py`` ``io_callback`` + HDF5/ArrayRecord sink unification; jaxbeans **DEPEND** packages (``safe_map`` import swap, multipart writers beyond manifest JSON); optional ``OUTPUT_SINKS`` registry + full driver breadth; **5h** ensemble relocation (Q3). |
