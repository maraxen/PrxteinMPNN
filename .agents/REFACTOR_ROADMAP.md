...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5-sprint-20260506` — **landed PR3a (2026-05-06):** ``run/sampling.py`` — ``_noop_sampling_structure_batch_io`` + tail ``io_callback(..., ordered=False)`` on ``_sample_batch`` / ``_sample_batch_averaged``; ``emit_structure_batch_io`` gates campaign HDF5 + ArrayRecord chunk loops (**last chunk only**); non-streaming ``sample()`` / ``_sample_averaged_mode`` — **concat-anchor** ``jax.effects_barrier()`` before ``jnp.concatenate`` (**removed** per-batch barrier after ``_sample_batch``); streaming HDF5/AR **sink** barriers after batch compute unchanged. Tests: ``tests/streaming/test_sampling_chunk_io_callback.py``. **Next:** PR3b scoring tensor D2H; **5e-cont**; **5f**; jaxbeans DEPEND. **Prior:** PR2c (`refactor-phase5-sprint-20260509`). |
| **Last update** | **2026-05-06** — Phase **5g PR3a** sampling structure-batch markers + barrier alignment. **Still defer:** tensor/chunk D2H streaming; HDF5 vs ArrayRecord adapter; jaxbeans package wiring; ``OUTPUT_SINKS``; **5h**. |
