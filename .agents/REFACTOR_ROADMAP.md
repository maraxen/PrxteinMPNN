...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5-streaming-contract-20260506` — **Plan gate:** recon → planner → plan-auditor (**NEEDS_WORK** → amended); **landed:** explicit ``_sample_batch`` JIT-tail / pseudo-perplexity contract in ``run/sampling.py`` + ``TODO_io_callback.txt``; ``DesignArrayRecordWriter.write()`` NumPy fast path (skip redundant ``device_get``); ``tests/streaming/test_design_writer_numpy.py``; ``tests/run/test_multistate_runtime_plumbing.py`` patches ``ScoringDriver.build_score_single_pair`` with ``functools.partial`` so multi-state kwargs match production. **Verification:** ``.agents/verification_logs/sprint_phase5_streaming_contract_20260506.filtered.txt`` (40 passed). Prior: ``refactor-phase5-closure-20260506``. |
| **Last update** | **2026-05-06** — Incremental **5g** writer + contract slice (not full JIT-tail ``io_callback`` inside ``_sample_batch``). **Still deferred:** concat-breaking D2H streaming; jaxbeans **DEPEND** swap; ``OUTPUT_SINKS`` registry; **5h**. |
| **Current phase** | Phase 5: **5g** documentation + host-array staging polish; **5f** test harness aligned with ``ScoringDriver`` partial binding. |
| **Still open** | Deep ``run/sampling.py`` ``io_callback`` + HDF5/ArrayRecord sink unification (chunk loop retains ``jnp.concatenate`` until redesigned with ppx / ``return_logits`` contract); jaxbeans **DEPEND** packages (``safe_map`` import swap, multipart writers beyond manifest JSON); optional ``OUTPUT_SINKS`` registry + full driver breadth; **5h** ensemble relocation (Q3). |
