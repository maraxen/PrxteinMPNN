...
| **Last update** | **2026-05-06** — **Phase 5f narrow:** ``run/sampling.sample()`` builds JIT ``sampler_fn`` via ``SamplingDriver(spec).build_sampler_fn(...)`` (registry path); verification ``phase5f_sample_driver_20260506.filtered.txt``. Same date — Phase 4 routing tests + ``sampler_factory_keys``. **Still not started:** mpnn split (5a–5e), io_callback streaming (5g), jaxbeans ``safe_map`` DEPEND swap. |
