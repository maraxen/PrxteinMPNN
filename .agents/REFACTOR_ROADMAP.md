...
| Field | Value |
| :--- | :--- |
| **task_id** | `refactor-phase5a-encoder-20260506` — Phase 5a protein encoder orchestration helpers (2026-05-06); prior `refactor-roadmap-sprint-20260506` slices (routing, driver keys, 5f sample path). |
| **Last update** | **2026-05-06** — **Phase 5a (narrow):** ``model/encoder.py`` adds ``pack_encoder_context`` + ``encoder_forward_with_int_neighbors``; ``PrxteinMPNN`` AR scan + protein ``state_vmap_exact`` encode/stack paths call them. **Ligand** duplicate packing remains inline (**Phase 5e** mirrors: ``mpnn.py`` ~2898 AR context, ~3529 wave ``encoder_stack``). Verification: ``phase5a_encoder_helpers_20260506.filtered.txt``. **Earlier same day:** Phase 5f ``SamplingDriver.build_sampler_fn`` in ``sample()``; Phase 4 routing tests + ``sampler_factory_keys``. |
| **Current phase** | Phase 4: routing regression tests landed; **remaining Phase 4 chunk** = optional ``state_vmap_exact`` **body** dedup / vmap reference extension for AR wave and deeper ligand paths (beyond routing parity). Phase 5: **5a started** (helpers + protein wiring); **still open:** decoder/core/shared splits (5b–5e), ligand orchestration dedup at pinned mirrors, io_callback streaming (5g), jaxbeans ``safe_map`` DEPEND swap. |
| **Still open** | ``state_vmap_exact`` implementation unification where desired; **remaining** ``run/sampling.py`` internals (_sample_streaming variants, averaged path wiring) vs full host-driver consolidation; ``utils/_vendored_callbacks.py`` + barriers. **Phase 5e:** fold LigandMPNN encoder packing duplicates into shared helpers (see §14 mirror line anchors). **Deferred to Phase 6 §15:** batch layout policy. |
