...
2. **Jaxlint advisory CI:** **Separate workflow** `.github/workflows/jaxlint-advisory.yml`, **`continue-on-error: true` on the job**, install attempts `uv pip install jaxlint`; if install or `uv run jaxlint .` fails, the step exits 0 after printing a skip reason (no red mainline). **Pin:** none until a stable distribution exists; document “unpinned optional try”.
...
5. **§14 roadmap success (one sentence):** “Phase 0 is closed when §14 lists no remaining *blocking* closeout items for scaffolding, or each remaining item is explicitly deferred with owner/trigger.” This sprint moves jaxbeans-dep, jaxlint CI, baseline capture, and 0a extension toward that bar.
...
| P0C-JAXLINT | `.github/workflows/jaxlint-advisory.yml` | Workflow syntax; optional manual `act` / GHA |
