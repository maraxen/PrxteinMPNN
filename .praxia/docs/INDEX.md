...
## Specs
- [260826_chain-selection-gap-closure](specs/260826_chain-selection-gap-closure.md) — **DRAFT** (task `260826_chain-selection-vendor-superset-audit`) — closes the 4 gaps found by the chain-selection vendor-superset audit against LigandMPNN. G1 (shovel-ready): generic `MappedBy`/`resolve_mapped_by` batch-input-mapping primitive, `by="path"` only implemented now, `NotImplementedError` on any other `by` value; widens `fixed_mask`/`fixed_tokens` to accept it. G2 (shovel-ready, one open owner-decision on precedence with explicit `fixed_mask`): `chains_to_design` chain-letter design/fix selector, resolved at prep-time from `chain_index`. G3/G4 (`symmetry_weights`, `homo_oligomer`) explicitly BLOCKED on tracing `tie_group_map`/`tied_positions` reachability first — not spec'd. Generic xtrax port of `MappedBy` tracked as deferred debt, out of scope here.
