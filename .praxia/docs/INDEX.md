...
## Specs
- [260706_samples-axis-planner-cardinality-mismatch](specs/260706_samples-axis-planner-cardinality-mismatch.md) — **DRAFT, finding only** — `n_samples` axis planning (`spec.samples_batch_size`, drives the memory-budget check) and the actual runtime sample count (`spec.samples_chunk_size`/`num_samples`, drives what's actually vmapped) are two independent `SamplingSpecification` fields with zero cross-validation; when the plan picks Vmap, `safe_map`'s `batch_size==0` branch ignores the real array size entirely, silently bypassing the budget check. Not part of EPIC #1541 — predates it. Remediation not yet chosen.
