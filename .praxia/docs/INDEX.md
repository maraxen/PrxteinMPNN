...
## Specs
- [260706_epic1541-p4-runner-hostsinks-scoping](specs/260706_epic1541-p4-runner-hostsinks-scoping.md) — **DRAFT** — EPIC #1541 P4 scoping: recon against real xtrax 0.4.0a1 API found "move output_sinks/streaming_host to xtrax" mostly doesn't apply (boundary types already xtrax-sourced, concrete sinks are domain-specific, `BoundedCallbackHandler`/`async_indexed_stream` naming collision is not a migration); the one genuinely open piece is `StageSet`/`xtrax.stages.bundle.StageBundle` composition (structural incompatibility verified, real blast radius across the decode pipeline).
