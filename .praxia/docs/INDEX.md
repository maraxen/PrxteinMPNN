...
- [260706_carryspec-dedupspec-stay-local-carryshape-migrates](decisions/260706_carryspec-dedupspec-stay-local-carryshape-migrates.md) — **Accepted 2026-07-06**: EPIC #1541 P3 — `CarryShape` migrates to xtrax (clean drop-in); `CarrySpec`/`DedupSpec` stay local (companions to the local-by-design planner; migrating would drop a heterogeneous-axis guard or break `kernel_dispatch.py`'s DedupGather isinstance check).
