...
**UPDATE 2026-07-20 (PR #124): process isolation implemented, one PyTorch number recovered.**
`heterogeneous_batch_benchmark.py` now re-invokes itself per strategy via `--strategy`, running each
of the three in its own subprocess so a JAX crash's corrupted CUDA context dies with its own process
instead of poisoning the ones that follow. Two bugs were found and fixed while validating this on
real cluster hardware before trusting it: (1) the first version still built *both* frameworks' models
in every subprocess regardless of which strategy was requested, and JAX's default ~75% GPU
preallocation on first device touch starved PyTorch even in "PyTorch-only" subprocesses — fixed to
only build the model the requested strategy needs; (2) a genuine Blackwell/SM120 driver quirk,
confirmed via direct `ps`/`nvidia-smi` polling: nvidia-smi keeps reporting a process's large CUDA
context as "in use" for several minutes after that process has already exited (`ps -p <pid>` shows
nothing) — same family as this section's already-documented "leaking" behavior, just triggered here
by ordinary large-context teardown, not only by the illegal-address fault. Added OOM retry-with-
backoff (up to 5 minutes cumulative) to ride out that window.

Full production-scale re-run (n=256, n_repeats=10, SLURM job `18378329`, node4007):
- `jax_bucket_pad_tile`: hits the same already-documented `CUDA_ERROR_ILLEGAL_ADDRESS` autotuning
  crash as every prior attempt (§7 below) — confirmed unrelated to this fix, still an open external
  compiler limitation with no upstream fix as of this session.
- `pytorch_pad_to_batch_max`: genuinely out-of-memory — padding this real 256-structure heterogeneous
  batch to its max present length (512) needs ~128GB, more than the node's entire 95GB GPU. This is a
  real, honest result about the naive strategy's infeasibility at this scale, not a bug or a
  contamination artifact — arguably stronger evidence for the padding-waste comparison this benchmark
  exists to make than a clean number would have been.
- `pytorch_per_structure_loop`: **succeeded**, real production number recovered: 2454.2ms ± 0.83ms
  wall-clock, 53,733 real elements, zero padding overhead (by construction). This is the first
  production-scale number this benchmark has ever produced for either framework.

Net: 1 of the 2 previously-unobtainable PyTorch numbers recovered; the JAX number and the naive
PyTorch strategy's number both remain blocked, but now for two distinct, well-characterized, and
clearly non-code reasons (an external compiler bug, and a real memory ceiling) rather than mutual
contamination.
...
| §7 real data | **All corrected 2026-07-18 (device-placement fix, jobs `18284162`+`18293715`)**: `outputs/ebm_benchmarks/{decoy,ddg}_benchmark_full_L{64-128,256,512}.json` (real GPU-vs-GPU rows + 1 expected OOM `error` row each at the largest cell), `biasing_benchmark_full.json`, `langevin_benchmark_full_L{64-128,256,512}.json` (1 expected OOM `error` row per file at the largest cell), `langevin_annealing_benchmark_full.json` — all real production-scale, real-GPU data for both frameworks. `heterogeneous_batch_benchmark_full.json` — **updated 2026-07-20 (job `18378329`, PR #124)**: `pytorch_per_structure_loop` now has a real production-scale row (2454.2ms ± 0.83ms, 53,733 real elements); `jax_bucket_pad_tile` and `pytorch_pad_to_batch_max` remain error rows, but for two distinct, non-code reasons (external compiler bug; genuine 128GB memory requirement) rather than mutual contamination — see §7 update above. |
