...
const trackM = () =>
  track(
    ...
    `task_id: ${TASK_ID}. task_id: 260605_multistate-potts | Requires B(1303) ADR complete. Mechanically enforce import boundary. Forbidden in aminx.potts.{model,poe,sampling}: aminx.inference.decode, aminx.host.plan, aminx.types.stages, aminx.inference.logits. aminx.potts.designer is EXEMPT. Implement Option A (ruff banned-api if supported in pyproject.toml), Option B (.ast-grep/rules/potts-import-boundary.yml if ast-grep available), and ALWAYS Option C: create tests/potts/test_import_boundaries.py — import ast,pathlib; FORBIDDEN=['aminx.inference.decode','aminx.host.plan','aminx.types.stages','aminx.inference.logits']; GUARDED=['src/aminx/potts/model.py','src/aminx/potts/poe.py','src/aminx/potts/sampling.py']; EXEMPT=['src/aminx/potts/designer.py']; for each guarded file: parse AST, assert no import from FORBIDDEN with message citing ADR 260605_potts-parallel-not-stageset. Wire into pytest: uv run pytest tests/potts/test_import_boundaries.py -x.\n\n${EMITTER_CTX}`,
    `task_id: ${TASK_ID}. Verify: (1) at least one enforcement mechanism (ruff/ast-grep/import-test); (2) forbidden list covers all 4 modules; (3) designer.py explicitly EXCLUDED from guard; (4) uv run pytest tests/potts/test_import_boundaries.py -x passes on clean source; (5) if ast-grep: scan reports no violations on model.py. PASS if criteria 1-4 met. FAIL if designer exemption missing.`,
  );
...
const trackF = () =>
  track(
    ...
    `task_id: ${TASK_ID}. task_id: 260605_multistate-potts | Source: /home/marielle/projects/mistypotts/src/mistypotts/exact_potts.py:98-118 (exact_marginals_bruteforce), potts_gibbs.py:12-50 (potts_log_unnormalized). Create tests/potts/test_potts_correctness.py. Group 1 — TRW vs bruteforce: @pytest.mark.parametrize('n',[6,8,10,12]) def test_trw_marginals_vs_exact(n): build random PottsModel (dense_pinv), run infer_params on random coords, run exact_marginals_bruteforce(h,J,W), assert jnp.max(jnp.abs(trw_marginals-exact_marginals))<0.05. Group 2 — x2 scale synthetic: def test_x2_scale_factor_two_residue(): construct known h(2,20),J(2,2,20,20) by hand; compute all 400 log_unnormalized, logsumexp->log_Z; for fixed seq assert PottsModel.log_prob(seq,h,J,W,mask) matches expected within 1e-5. Group 3 — alphabet: def test_potts_vs_mpnn_alphabet_ordering(): import MPNN_ALPHABET from aminx.utils.aa_convert; import POTTS_ALPHABET from aminx.potts.model; assert MPNN_ALPHABET[:20]==POTTS_ALPHABET[:20] else fail with diff table. All tests: @pytest.mark.potts.\n\n${EMITTER_CTX}`,
    `task_id: ${TASK_ID}. Verify tests/potts/test_potts_correctness.py: (1) Three groups present (TRW parametrized n=[6,8,10,12], x2 synthetic, alphabet); (2) TRW tolerance 0.05; (3) x2 test constructs h,J by hand, expected log_prob analytical, asserts 1e-5; (4) alphabet test imports POTTS_ALPHABET, fails with diff table if mismatch; (5) all use @pytest.mark.potts; (6) uv run pytest tests/potts/test_potts_correctness.py -m potts --collect-only exits 0 (collects 6 tests). PASS if all 6 met.`,
  );
...
const trackN = () =>
  track(
    ...
    `task_id: ${TASK_ID}. task_id: 260605_multistate-potts | BATHOS rule: tracked script, not inline snippet. Create scripts/validate/poe_energy_sanity.py: argparse (--n-residues default 6, --num-aa default 20, --seed default 0, --out default /dev/null), logging. Construct random PottsModel (tiny n=6,num_aa=20,hidden_dim=16); run infer_params on random coords->PottsParams(h,J,W); fixed seq=jnp.zeros(6,dtype=int). ASSERTION 1: PoeModel(n_backbones=2,identical backbones).joint_energy(seq,[params,params]) - 2*potts_log_unnormalized(h,J,W,seq,mask) < 1e-5. ASSERTION 2: given h_raw,J_raw (before x2), verify potts_log_unnormalized(2*h_raw,2*J_raw,W,seq,mask) matches expected within 1e-6. Log pass/fail with exact values; exit 0 on pass, exit 1 on failure. If PoeModel not yet implemented: exit with NotImplementedError (not crash). Create scripts/validate/poe_energy_sanity.bth.toml (bathos sidecar with run_id,script,description,hypotheses).\n\n${EMITTER_CTX}`,
    `task_id: ${TASK_ID}. Verify: (1) both .py and .bth.toml exist; (2) Assertion 1: joint_energy≈2*single_energy tolerance 1e-5; (3) Assertion 2: x2 scale explicitly verified; (4) bathos sidecar valid schema; (5) uv run python scripts/validate/poe_energy_sanity.py --out /dev/null exits 0 (or clean NotImplementedError); (6) proper .py with argparse, not heredoc. PASS if all 6 met.`,
  );
