"""Central registries and frozen dispatch tables for aminx.

``SAMPLERS`` holds sampler **factories** (``model, …`` →
:class:`~aminx.protocols.SamplerFn`). ``OUTPUT_SINKS`` holds **builders**
(``()`` → :class:`~aminx.protocols.DesignSink`) for host tensor staging.
"""
...
from aminx.types.protocols import DesignSink, SamplerFn
...
import aminx.host.output_sinks as _output_sinks_bootstrap  # noqa: F401 — register default sink keys
