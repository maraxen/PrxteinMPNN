...
from prxteinmpnn.tiling.iterator import JaxScanIterator
from prxteinmpnn.tiling.strategy import AxisStrategy
...
def make_decode_fn(
    ...
    try:
        ...
        elif isinstance(mode, UnconditionalMode):
            state_iter = make_axis_dispatch(strategy, axis="state")
            return UnconditionalDecode(model=model, state_iterator=state_iter)

        elif isinstance(mode, AutoregressiveMode):
            state_iter = make_axis_dispatch(strategy, axis="state")
