...
from prxteinmpnn.inference.decode._base import _ConditionalDecodeBase
from prxteinmpnn.inference.decode._kernel import _tied_group_einsum_average
    ...
    def __call__(
        ...
        if fixed_mask is not None and fixed_tokens is not None:
            ...
            fixed_one_hot = jax.nn.one_hot(
                fixed_tokens, num_classes, dtype=jnp.float32
            )
        ...
        def loss_fn(logits: Logits) -> jnp.ndarray:
            ...
            if use_concrete:
                ...
                progress = jnp.float32(0) / jnp.maximum(
                    jnp.float32(self.iterations) - 1.0, 1.0
                )
                tau = jnp.float32(tau_start) * (
                    jnp.float32(tau_end / tau_start) ** progress
                )
                key_gumbel = jax.random.fold_in(prng_key, 0)
                seq_repr = gumbel_softmax(
                    logits / temperature, tau, key_gumbel, hard=True
                )
        ...
        def update_step(
            ...
            if tie_group_map is not None and num_groups is not None:
                ...
                next_logits_averaged = _tied_group_einsum_average(
                    next_logits_expanded, tie_group_map[0], num_groups
                )
