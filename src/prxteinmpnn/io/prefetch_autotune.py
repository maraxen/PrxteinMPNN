...
def _get_element_size_bytes(element: Any) -> int:  # noqa: ANN401
...
def _get_average_element_size_mb(
  ...
  elements_to_sample.extend(
    itertools.chain.from_iterable(
      (itertools.islice(ds, samples_per_ds) for ds in dss),
    ),
  )
  ...
  return float(avg_size_bytes) / (1024 * 1024)
...
def _get_buffer_size(
  ...
  *,
  ...
  """Analyze elements' sizes to choose an optimal buffer size for a PrefetchIterDataset."""
...
class PerformanceConfig:
  """Configuration for optimal performance of a MultiprocessPrefetchIterDataset."""
...
def pick_performance_config(
  ...
  *,
  ...
  """Analyze element size to choose an optimal number of workers.
