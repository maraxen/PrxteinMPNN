...
class PrxteinLigandMPNN(eqx.Module):
  ...
  def __call__(self, key: PRNGKeyArray, inputs: ModelInputs, **kwargs: Any) -> Any:
    ...
    if isinstance(inputs, UnconditionalInputs):
      return self.score_unconditional_from_payload(
          key, inputs.state_stack, ligand=inputs.ligand, **kwargs
      )
    ...
    *,
    ...
    if ms_route.uses_stacked_exact_model_call:
      ...
      need_lm = {
        "coords_stack": coords,
        "mask_stack": mask,
        "residue_index_stack": residue_index,
        "chain_index_stack": chain_index,
        "y_stack": y,
        "y_t_stack": y_t,
        "y_m_stack": y_m,
      ...
      if decoding_approach == "conditional":
        if one_hot_sequence is None:
          ...
          raise ValueError(msg)
        s_dim_lm, p_dim_lm = mask.shape[0], mask.shape[1]
        arm_lm = (
  ...
  def score_unconditional_from_payload(
    ...
    *,
    ...
    """Unconditional scoring from ProteinBundle + LigandBundle."""
    if ligand is None:
  ...
  def score_conditional_from_payload(
    ...
    *,
    ...
    """Conditional scoring from ProteinBundle + LigandBundle."""
    if ligand is None:
  ...
  def sample_autoregressive_state_vmap_exact_from_payload(
    ...
    *,
    ...
    """Same as :meth:`sample_autoregressive_state_vmap_exact` with ``stack`` + :class:`~prxteinmpnn.bundles.LigandBundle`."""
    if ligand is None:
