...
def process_shard(
    ...
    # Pre-load forcefield once per worker
    force_field = load_force_field_from_hub(force_field_name)
    
    record_count = 0
    
    for pt_file in files:
        # --- FIX: Derive Name from Filename ---
        # e.g., "src/data/1abc_A.pt" -> "1abc_A"
        # This matches the 'CHAINID' column in your list.csv
        ...
        for i, protein in enumerate(proteins):
            try:
                # Validation checks (skip assembly metadata files that lack coords)
                if 'seq' not in protein or 'xyz' not in protein: 
                    continue
                
                # Use filename as ID. If file contains multiple proteins (rare for this dataset),
                # append index to ensure uniqueness.
                if len(proteins) > 1:
                    name = f"{file_stem}_{i}"
                else:
                    name = file_stem
                ...
                for r_i, r_idx in enumerate(seq):
                    ...
                    for a_i, atom_name in enumerate(atom_types):
                        ...
                        charges[r_i, a_i] = q
                        sigmas[r_i, a_i] = sig
                        epsilons[r_i, a_i] = eps
                        radii[r_i, a_i] = rad

                # --- Serialize ---
                record_data = {
                    "protein_id": name,  # Now contains "1abc_A"
                ...
                logger.exception(f"Error processing file: {pt_file}")
                continue
