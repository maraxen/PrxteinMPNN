"""
Final test with comprehensive weight loading.
"""
...
# Load models
print("\n" + "="*80)
print("Loading models with COMPREHENSIVE weight loading")
print("="*80)

colab_weights_path = "/tmp/ColabDesign/colabdesign/mpnn/weights/v_48_020.pkl"
key = jax.random.PRNGKey(42)
prxtein_model = load_prxteinmpnn_with_colabdesign_weights(colab_weights_path, key=key)
...
print(f"ColabDesign logits (MPNN order) shape: {colab_logits_mpnn.shape}")
print(f"ColabDesign logits (MPNN order) range: [{colab_logits_mpnn.min():.3f}, {colab_logits_mpnn.max():.3f}]")
...
print("="*80)
if corr > 0.9:
    print("✅ PASS: Correlation >0.9 - Models match!")
elif corr > 0.8:
    print(f"🟡 CLOSE: Correlation {corr:.4f} - Almost there!")
else:
