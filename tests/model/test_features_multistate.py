...
def test_features_structure_mapping_prevents_cross_neighbors(features_module):
  ...
  edge_features, neighbor_indices, _ = features_module(
    prng_key,
...
def test_features_without_structure_mapping_allows_cross_neighbors(features_module):
...
def test_features_structure_mapping_backward_compatible(features_module):
  ...
  _, neighbors_none, _ = features_module(
    prng_key,
    protein.coordinates,
    protein.atom_mask,
  ...
  _, neighbors_mapped, _ = features_module(
    prng_key,
    protein.coordinates,
    protein.atom_mask,
    protein.residue_index,
...
def test_features_structure_mapping_shape_validation(features_module):
...
def test_features_structure_mapping_jit_compatible(features_module):
  ...
  edge_features_jit, neighbors_jit, _ = jitted_features(
    prng_key,
    protein.coordinates,
    protein.atom_mask,
    protein.residue_index,
  ...
  edge_features_nojit, neighbors_nojit, _ = features_module(
    prng_key,
    protein.coordinates,
    protein.atom_mask,
    protein.residue_index,
...
def test_features_structure_mapping_multiple_structures(features_module):
...
def test_features_structure_mapping_with_backbone_noise(features_module):
...
def test_features_structure_mapping_edge_feature_shapes(features_module):
  ...
  # Check shapes
  assert edge_features.shape[0] == n_residues, "Edge features first dim should match residues"
  assert neighbor_indices.shape[0] == n_residues, "Neighbor indices first dim should match residues"
  assert neighbor_indices.shape[1] <= features_module.k_neighbors, (
    "Too many neighbors returned"
  )
