...
def test_mpnn_autoregressive_with_structure_mapping(mpnn_model):
  ...
  # Verify output shape
  assert logits.shape == (protein.coordinates.shape[0], 21)  # (N, 21 amino acids)

  # Verify logits are finite
  assert jnp.all(jnp.isfinite(logits))


def test_mpnn_single_sequence_with_structure_mapping(mpnn_model):
...
def test_mpnn_with_backbone_noise_and_structure_mapping(mpnn_model):
...
def test_mpnn_without_structure_mapping_backward_compatible(mpnn_model):
  ...
  _, logits_no_mapping = mpnn_model(
    protein.coordinates,
    protein.atom_mask,
    protein.residue_index,
    protein.chain_index,
  ...
  _, logits_single_structure = mpnn_model(
    protein.coordinates,
    protein.atom_mask,
    protein.residue_index,
    protein.chain_index,
  ...
  assert jnp.allclose(logits_no_mapping, logits_single_structure, atol=1e-5)





def test_mpnn_multiple_structures_isolation(mpnn_model):
...
def test_mpnn_jit_compatible_with_structure_mapping(mpnn_model):
