...
def test_split_path_with_tied_positions(model_inputs, rng_key):
  ...
  assert seq[0] == seq[1], "Positions 0 and 1 should be identical"
  assert seq[0] == seq[2], "Positions 0 and 2 should be identical"
  assert seq[5] == seq[6], "Positions 5 and 6 should be identical"

  # Check sequence is valid
  assert seq.dtype == jnp.int8
  assert seq.shape == (n_residues,)
  assert jnp.all((seq >= 0) & (seq < 21))


def test_split_path_without_tied_positions(model_inputs, rng_key):
...
def test_split_path_consistency_with_full_path(model_inputs, rng_key):
  ...
  sample_full_fn = make_sample_sequences(model, sampling_strategy="temperature")
  ...
  assert seq_split[0] == seq_split[1], "Split path: positions 0,1 should be tied"
  assert seq_split[2] == seq_split[3], "Split path: positions 2,3 should be tied"

  # Check full path respects ties
  assert seq_full[0] == seq_full[1], "Full path: positions 0,1 should be tied"
  assert seq_full[2] == seq_full[3], "Full path: positions 2,3 should be tied"


def test_split_path_with_temperature(model_inputs, rng_key):
  ...
  assert seq_low_temp[0] == seq_low_temp[1], "Low temp: tied positions should match"
  assert (
    seq_high_temp[0] == seq_high_temp[1]
  ), "High temp: tied positions should match"

  # Both should be valid sequences
  assert jnp.all((seq_low_temp >= 0) & (seq_low_temp < 21))
  assert jnp.all((seq_high_temp >= 0) & (seq_high_temp < 21))
