...
def test_ste_optimization_with_tied_positions(model_inputs, rng_key):
    ...
    assert jnp.allclose(seq[0], seq[1])
    assert jnp.allclose(seq[2], seq[3])
    assert jnp.allclose(seq[4], seq[5])
    assert jnp.allclose(seq[6], seq[7])
    assert jnp.allclose(seq[8], seq[9])


def test_ste_optimization_without_tied_positions(model_inputs, rng_key):
